...
# Modify the process function to return the extracted graphemes
def process(args):
    args = args.replace(" ", "")
    args = normalize_word(args)
    if "র্" in args or "্র" in args or '্য' in args:
        return ads_grapheme_extraction(args)
    else:
        return vds_grapheme_extraction(args)

# Read the CSV file
original_filename = '/Users/marufahmed/Work/Apurba/ocr_dataset_evaluation/statistics/new_statistics/content/char/18_tw_char_to_page.csv'
data = pd.read_csv(original_filename)

# Accumulate all extracted graphemes
all_graphemes = []
for text in data['chars_supervisor']:
    extracted_graphemes = process(text)
    all_graphemes.append(extracted_graphemes)

# Create a DataFrame with all graphemes
all_graphemes_df = pd.DataFrame({'chars_supervisor': data['chars_supervisor'], 'Extracted_Graphemes': all_graphemes})

# Check if Extracted_Graphemes has multiple entries
all_graphemes_df['Multiple_Graphemes'] = all_graphemes_df['Extracted_Graphemes'].apply(lambda x: len(x) > 1)

# Split DataFrame into two based on the condition
multiple_graphemes_df = all_graphemes_df[all_graphemes_df['Multiple_Graphemes']]
single_grapheme_df = all_graphemes_df[~all_graphemes_df['Multiple_Graphemes']]

# Remove the 'Multiple_Graphemes' column
multiple_graphemes_df.drop(columns=['Multiple_Graphemes'], inplace=True)
single_grapheme_df.drop(columns=['Multiple_Graphemes'], inplace=True)

# Get the original filename without extension
filename_without_extension = os.path.splitext(os.path.basename(original_filename))[0]

# Save multiple graphemes to a new CSV file with 'grapheme' appended to the original filename
multiple_graphemes_df.to_csv(f'{filename_without_extension}_grapheme.csv', index=False, encoding='utf-8-sig')

# Save single grapheme to a new CSV file with 'character' appended to the original filename
single_grapheme_df.to_csv(f'{filename_without_extension}_character.csv', index=False, encoding='utf-8-sig')